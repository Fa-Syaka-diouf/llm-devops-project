...
Au-delà des exigences obligatoires, quatre améliorations supplémentaires ont été implémentées : la restructuration des manifestes avec Kustomize (environnements dev/prod), des NetworkPolicy, des PodDisruptionBudget, et un pipeline de validation continue (CI) sur GitHub Actions. Pour ces raisons de robustesse, aucune de ces fonctionnalités n'a été appliquée directement sur le cluster Minikube déjà validé (sections précédentes) : le risque de saturer à nouveau les ressources de la machine (voir section~\ref{sec:incidents}) n'en valait pas la peine. Chaque élément a en revanche été rendu et validé localement (\texttt{kubectl kustomize} + \texttt{kubeconform}), puis vérifié en conditions réelles via le pipeline CI décrit en~\ref{sec:cicd}.

\subsection{Restructuration Kustomize (environnements dev / prod)}
